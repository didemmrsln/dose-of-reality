...
-- int_ingredients_normalized (mcg'ye standardize edilmis) tablosuna
-- UL referans degerlerini join ediyoruz.
...
select
    ...
    norm.unit_label,
    norm.quantity_mcg,
    norm.unit_convertible,
    norm.category,

    ul.ul_value,
    ul.unit as ul_unit_raw,
    ...
    ul.age_group as ul_age_group,
    ul.scope_note as ul_scope_note,
    case when ul.ul_value is not null then true else false end as has_ul_defined

from {{ ref('int_ingredients_normalized') }} as norm
